...
&emsp;&emsp;出于我们小组在概念设计阶段对该大作业——中大校园食堂的菜品评价系统**中大食评**的功能需求的考虑，包括在实现中期时经过协商讨论决定增加的数据库实体，我们小组的数据库最终呈现中包含了6个实体，分别命名为 Campus (校区)、Cafeteria (餐厅)、Dish (菜品)、ServedTime (供应时间)、Comments (评价) 和 User (用户)。接下来的数据库设计主要拆分成对具体实体的属性设计分析，和实体-联系模型的设计分析两个部分进行讲解。
